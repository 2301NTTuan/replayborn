...
func run() -> void:
	...
	check(pause.size == Vector2(182, 56), "pause matches the replay tile height")
	check(pause.position + pause.size <= pause.get_parent().size, "pause remains inside the run panel")
	check(game.hud.echo_label.get_parent() == pause.get_parent(), "replay counter shares the run panel")
	game.queue_free()
	await process_frame
